...
namespace DotNetSpy
{
    ...
    public class WindowPropertiesView : System.Windows.Forms.UserControl, IHookInstall
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.propertyGrid = new DotNetSpy.PropertyGridEx();
        ...
        private void CreateInstance(IntPtr hWndParent)
        {
            try
            {
                IntPtr hwnd = NativeMethods.GetWindow(hWndParent, (int)GetWindowCmd.GW_CHILD);
                if (IntPtr.Zero != hwnd)
                {
                    Control control = Control.FromHandle(hwnd);
                    if (control != null && control is WindowPropertiesView)
                    { 
